...
class AiClient
{
    ...
    /**
     * Combine Structured Exceptions To Repetitive Exception
     *
     * This method is used when we already have multiple structured exceptions
     * and we want to merge them into one repetitive exception record.
     *
     * Input: multiple messages / structured exception outputs
     * Output: one combined result: short message + detailed message + severity + carrier + internal/external
     *
     * This is the step that helps us say:
     * "These 10 exceptions are actually the same recurring problem."
     */
    public function combineStructuredExceptionsToRepetitiveException(array $messages): array {

        /**
         * We again define a strict schema so the AI returns predictable output.
         */
        $schema = new ObjectSchema(
            name: 'exception_classification',
            description: 'Classification of a Laravel exception',
        ...
        /**
         * Prompt for "combining" rather than "classifying".
         * We tell it to merge multiple messages into one.
         */
        $prompt = "
        ...
        /**
         * Send the combine request to the AI.
         * This can take longer because it processes multiple messages,
         * so we use a bigger timeout.
         */
        $response = Prism::structured()
            ->using(Provider::Ollama, 'mistral:latest')
            ->withSchema($schema)
        ...
        // Return the combined structured result as an array.
